...
class _SimpleTimePickerState extends State<SimpleTimePicker> {
  ...
  List<int> getHours() {
    ...
    // لو اليوم هو النهارده → الساعات تبدأ من الساعة الحالية
    // لو لأ → من 0 إلى 23
    if (isToday) {
      return List.generate(24 - now.hour, (index) => now.hour + index);
    } else {
  ...
  List<int> getMinutes() {
    if (isToday && selectedHour == TimeOfDay.now().hour) {
      // لو الساعة الحالية → يبدأ من الدقيقة الحالية
      final now = TimeOfDay.now();
      return List.generate(60 - now.minute, (index) => now.minute + index);
    } else {
  ...
  Widget build(BuildContext context) {
    ...
    final validHourIndex = hourIndex >= 0 ? hourIndex : 0;

    final minutes = getMinutes();
    final validMinuteIndex =
        minutes.contains(selectedMinute) ? minutes.indexOf(selectedMinute) : 0;
      ...
      child: Row(
        ...
        children: [
          ...
          SizedBox(
            ...
            child: ListWheelScrollView.useDelegate(
              controller:
                  FixedExtentScrollController(initialItem: validMinuteIndex),
              itemExtent: 40,
              perspective: 0.005,
              diameterRatio: 1.5,
              physics: const FixedExtentScrollPhysics(),
          ...
          SizedBox(
            ...
            child: ListWheelScrollView.useDelegate(
              controller:
                  FixedExtentScrollController(initialItem: validHourIndex),
              itemExtent: 40,
              perspective: 0.005,
              diameterRatio: 1.5,
              physics: const FixedExtentScrollPhysics(),
              ...
              onSelectedItemChanged: (index) {
                setState(() {
                  ...
                  // إعادة ضبط الدقائق عند تغيير الساعة
                  final mins = getMinutes();
